...
const isStackFromOldLocalFallback = RegExp.prototype.test.bind(
  /lib[\\/]sls3-legacy.js:\d+[^]+lib[\\/]sls3-legacy.js:\d+/i
);
  ...
  constructor(config = {}) {
    ...
    if (isStackFromOldLocalFallback(new Error().stack)) {
      ...
      throw new ServerlessError(
        [
          'Cannot run local installation of the SLS3-legacy by the outdated global version.',
          'Please upgrade via:',
          '',
          chalk.bold(
            isStandaloneExecutable ? 'sls3 upgrade --major' : 'npm install -g sls3-legacy'
          ),
          chalk.grey(
            'Note: Latest release can run any version of the locally installed SLS3-Legacy.'
          ),
          '',
          'Alternatively run locally installed version directly via:',
          '',
          chalk.bold('npx sls3 <command> <options>'),
        ].join('\n'),
